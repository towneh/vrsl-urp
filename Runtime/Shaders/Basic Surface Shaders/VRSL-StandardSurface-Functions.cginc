float GetSurfaceStrobe(uint DMXChannel)
{
    ...
    // float phase = getValueAtCoordsRaw(DMXChannel + 4, _VRSLU_DMXGridStrobeTimer);
    // float status = getValueAtCoords(DMXChannel + 4, _VRSLU_DMXGridRenderTexture);
...
{
    float redchannel = getValueAtCoords(DMXChannel + 1 + channelOffset, _VRSLU_DMXGridRenderTexture);
    float greenchannel = getValueAtCoords(DMXChannel + 2 + channelOffset, _VRSLU_DMXGridRenderTexture);
    float bluechannel = getValueAtCoords(DMXChannel + 3 + channelOffset, _VRSLU_DMXGridRenderTexture);
...
{
    return  IF(isDMX() == 1 && enableDMXControl == 1, getValueAtCoords(DMXChannel, _VRSLU_DMXGridRenderTexture), 0.0);
}
...
float GetDMX12CH(uint DMXChannel, int dimmerChannel)
{
    ...
    float dimmer = getValueAtCoords(startChannel, _VRSLU_DMXGridRenderTexture);
    return dimmer;
}
float4 GetDMX12CHRGB(uint DMXChannel, int dimmerChannel, out float intensity)
{
    ...
    intensity = getValueAtCoords(startChannel, _VRSLU_DMXGridRenderTexture);
    ...
    float4 GetDMXEmission1CH(float2 EmissionUV)
    {
        ...
        float dmxIntensity = IF(isDMX()==1, getValueAtCoords(dmx, _VRSLU_DMXGridRenderTexture), 1.0);
    ...
    {
        uint dmx = getDMXChannel();
        float dmxIntensity = IF(isDMX()==1, getValueAtCoords(dmx+5, _VRSLU_DMXGridRenderTexture), 1.0);
...
{
    uint dmx = getDMXChannel();
    //float dmxIntensity = getValueAtCoords(dmx, _VRSLU_DMXGridRenderTexture);
    #ifndef _CHANNEL_MODE
...
float GetDMXAlpha()
{
    return UNITY_ACCESS_INSTANCED_PROP(Props, _EnableDMXAlpha) == 1 && isDMX() == 1 ? getValueAtCoords((getDMXChannel() + 5), _VRSLU_DMXGridRenderTexture) : 1.0;
}
