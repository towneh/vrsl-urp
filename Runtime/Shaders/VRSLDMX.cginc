...
UNITY_INSTANCING_BUFFER_END(Props)
    ...
    Texture2D _VRSLU_DMXGridStrobeOutput, _VRSLU_DMXGridSpinTimer, _VRSLU_DMXGridRenderTextureMovement;
    uniform float4 _VRSLU_DMXGridStrobeOutput_TexelSize, _VRSLU_DMXGridSpinTimer_TexelSize, _VRSLU_DMXGridRenderTextureMovement_TexelSize;
    SamplerState VRSL_PointClampSampler;
#endif
...
float GetStrobeOutput(uint DMXChannel)
{
    ...
    #else
        //float phase = ReadDMXRaw(DMXChannel, _VRSLU_DMXGridStrobeTimer);
        half strobe = ReadDMX(DMXChannel, _VRSLU_DMXGridStrobeOutput);
...
float GetImmediateStrobeOutput(uint DMXChannel)
{
        ...
        return strobe;
    #else
        return ReadDMX(DMXChannel, _VRSLU_DMXGridStrobeOutput);
    #endif
...
float4 GetDMXColor(uint DMXChannel)
{
    ...
    #else
        float redchannel = ReadDMX(DMXChannel, _VRSLU_DMXGridRenderTexture);
        float greenchannel = ReadDMX(DMXChannel + 1, _VRSLU_DMXGridRenderTexture);
        float bluechannel = ReadDMX(DMXChannel + 2, _VRSLU_DMXGridRenderTexture);
    #endif
...
float GetPanValue(uint DMXChannel)
{
    ...
    #else
        float inputValue = ReadDMX(DMXChannel, _VRSLU_DMXGridRenderTextureMovement);
    #endif
...
float GetTiltValue(uint DMXChannel)
{
    ...
    #else
        float inputValue = ReadDMX(DMXChannel + 2, _VRSLU_DMXGridRenderTextureMovement);
    #endif
