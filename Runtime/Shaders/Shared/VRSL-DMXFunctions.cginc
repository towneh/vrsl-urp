...
half getMaxConeLength(uint DMXChannel)
{
    ...
    return isDMX() == 1 && _EnableExtraChannels == 1 ? mcl + (getValueAtCoords(DMXChannel+1, _VRSLU_DMXGridRenderTexture) * 4) : mcl;
    #else
...
half GetStrobeOutput(uint DMXChannel)
{
    ...
    // half phase = getValueAtCoordsRaw(DMXChannel + 6, _VRSLU_DMXGridStrobeTimer);
    // half status = getValueAtCoords(DMXChannel + 6, _VRSLU_DMXGridRenderTexture);

    half strobe = getValueAtCoords(DMXChannel + 6, _VRSLU_DMXGridStrobeOutput);
...
half GetStrobeOutputFiveCH(uint DMXChannel)
{
    ...
    // half phase = getValueAtCoordsRaw(DMXChannel + 4, _VRSLU_DMXGridStrobeTimer);
    // half status = getValueAtCoords(DMXChannel + 4, _VRSLU_DMXGridRenderTexture);

    half strobe = getValueAtCoords(DMXChannel + 4, _VRSLU_DMXGridStrobeOutput);
    // strobe = IF(strobe > 0.0, 1.0, 0.0);//turn to square wave
    //strobe = saturate(strobe);
...
half getDMXGoboSelection(uint DMXChannel)
{
    ...
    uint value = round(((getValueAtCoords(DMXChannel + 11, _VRSLU_DMXGridRenderTexture))*255)/goboSelect);
...
half getGoboSpinSpeed (uint DMXChannel)
{
        ...
        half status = getValueAtCoords(DMXChannel + 10, _VRSLU_DMXGridRenderTexture);
        half phase = getValueAtCoordsRaw(DMXChannel + 10, _VRSLU_DMXGridSpinTimer);
...
{
    return getValueAtCoords(DMXChannel + 5, _VRSLU_DMXGridRenderTexture) * multiplier;
}

half GetDMXChannel(uint DMXChannel)
{
    return getValueAtCoords(DMXChannel, _VRSLU_DMXGridRenderTexture);
}
...
half GetFinePanValue(uint DMXChannel)
{
    ...
    return getValueAtCoords(DMXChannel+1, _VRSLU_DMXGridRenderTextureMovement);
}
half GetPanValue(uint DMXChannel)
{
    half inputValue = getValueAtCoords(DMXChannel, _VRSLU_DMXGridRenderTextureMovement);
...
half GetFineTiltValue(uint DMXChannel)
{
    ...
    return getValueAtCoords(DMXChannel+3, _VRSLU_DMXGridRenderTextureMovement);
}

//function for getting the Tilt Value (Channel 3)
half GetTiltValue(uint DMXChannel)
{
    half inputValue = getValueAtCoords(DMXChannel + 2, _VRSLU_DMXGridRenderTextureMovement);
...
{
    half redchannel = getValueAtCoords(DMXChannel + 7, _VRSLU_DMXGridRenderTexture);
    half greenchannel = getValueAtCoords(DMXChannel + 8, _VRSLU_DMXGridRenderTexture);
    half bluechannel = getValueAtCoords(DMXChannel + 9, _VRSLU_DMXGridRenderTexture);
...
{
    half inputvalue = getValueAtCoords(DMXChannel + 4, _VRSLU_DMXGridRenderTexture);
