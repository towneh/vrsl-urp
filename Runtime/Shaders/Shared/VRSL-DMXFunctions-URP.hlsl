...
half getMaxConeLength(uint DMXChannel)
{
    ...
    return isDMX() == 1 && _EnableExtraChannels == 1 ? mcl + (getValueAtCoords(DMXChannel+1, TEXTURE2D_ARGS(_VRSLU_DMXGridRenderTexture, sampler_VRSLU_DMXGridRenderTexture)) * 4) : mcl;
    #else
...
{
    half strobe = getValueAtCoords(DMXChannel + 6, TEXTURE2D_ARGS(_VRSLU_DMXGridStrobeOutput, sampler_VRSLU_DMXGridStrobeOutput));
...
{
    half strobe = getValueAtCoords(DMXChannel + 4, TEXTURE2D_ARGS(_VRSLU_DMXGridStrobeOutput, sampler_VRSLU_DMXGridStrobeOutput));
...
half getDMXGoboSelection(uint DMXChannel)
{
    ...
    uint value = round(((getValueAtCoords(DMXChannel + 11, TEXTURE2D_ARGS(_VRSLU_DMXGridRenderTexture, sampler_VRSLU_DMXGridRenderTexture)))*255)/goboSelect);
...
half getGoboSpinSpeed (uint DMXChannel)
{
        ...
        half status = getValueAtCoords(DMXChannel + 10, TEXTURE2D_ARGS(_VRSLU_DMXGridRenderTexture, sampler_VRSLU_DMXGridRenderTexture));
        half phase = getValueAtCoordsRaw(DMXChannel + 10, TEXTURE2D_ARGS(_VRSLU_DMXGridSpinTimer, sampler_VRSLU_DMXGridSpinTimer));
...
{
    return getValueAtCoords(DMXChannel + 5, TEXTURE2D_ARGS(_VRSLU_DMXGridRenderTexture, sampler_VRSLU_DMXGridRenderTexture)) * multiplier;
}

half GetDMXChannel(uint DMXChannel)
{
    return getValueAtCoords(DMXChannel, TEXTURE2D_ARGS(_VRSLU_DMXGridRenderTexture, sampler_VRSLU_DMXGridRenderTexture));
}

//function for getting the Pan Value (Channel 2)
half GetFinePanValue(uint DMXChannel)
{
    return getValueAtCoords(DMXChannel+1, TEXTURE2D_ARGS(_VRSLU_DMXGridRenderTextureMovement, sampler_VRSLU_DMXGridRenderTextureMovement));
}
half GetPanValue(uint DMXChannel)
{
    half inputValue = getValueAtCoords(DMXChannel, TEXTURE2D_ARGS(_VRSLU_DMXGridRenderTextureMovement, sampler_VRSLU_DMXGridRenderTextureMovement));
    #if defined(VOLUMETRIC_YES) || defined(PROJECTION_YES) || defined(FIXTURE_EMIT) || defined(FIXTURE_SHADOWCAST) || defined(VRSL_SURFACE) || defined(VRSL_FLARE)
...
{
    return getValueAtCoords(DMXChannel+3, TEXTURE2D_ARGS(_VRSLU_DMXGridRenderTextureMovement, sampler_VRSLU_DMXGridRenderTextureMovement));
}

//function for getting the Tilt Value (Channel 3)
half GetTiltValue(uint DMXChannel)
{
    half inputValue = getValueAtCoords(DMXChannel + 2, TEXTURE2D_ARGS(_VRSLU_DMXGridRenderTextureMovement, sampler_VRSLU_DMXGridRenderTextureMovement));
    #if defined(VOLUMETRIC_YES) || defined(PROJECTION_YES) || defined(FIXTURE_EMIT) || defined(FIXTURE_SHADOWCAST) || defined(VRSL_SURFACE) || defined(VRSL_FLARE)
...
{
    half redchannel = getValueAtCoords(DMXChannel + 7, TEXTURE2D_ARGS(_VRSLU_DMXGridRenderTexture, sampler_VRSLU_DMXGridRenderTexture));
    half greenchannel = getValueAtCoords(DMXChannel + 8, TEXTURE2D_ARGS(_VRSLU_DMXGridRenderTexture, sampler_VRSLU_DMXGridRenderTexture));
    half bluechannel = getValueAtCoords(DMXChannel + 9, TEXTURE2D_ARGS(_VRSLU_DMXGridRenderTexture, sampler_VRSLU_DMXGridRenderTexture));
...
{
    half inputvalue = getValueAtCoords(DMXChannel + 4, TEXTURE2D_ARGS(_VRSLU_DMXGridRenderTexture, sampler_VRSLU_DMXGridRenderTexture));
