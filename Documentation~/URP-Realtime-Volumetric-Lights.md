...
- **Froxel** — scattering is integrated once into a view-aligned 3D grid by
  ...
  rather than layering on top of it. **Not a replacement for the raymarch modes — see
  *Choosing a volumetric mode* below.**
...
### Choosing a volumetric mode

`Half` is the default and is expected to stay the default. The froxel path is an option for a
specific case, not a general upgrade, and the reason is worth stating plainly because the
architecture argues the other way.

Froxel volumetrics amortise cost across **low-frequency** participating media — atmospheric
haze, fog banks, shafts through a window. VRSL's defining visual is the opposite: hard-edged
spotlight cones. A view-aligned grid at any affordable resolution cannot hold those edges.

At the default 160×90×64 against a 1000×800 viewport, the volume is roughly a 6× downsample
per axis before trilinear filtering. Measured against a 15-fixture scene:

| Mode | Froxels | Light evaluations | Volume memory (2 eyes) | Cone edges |
|---|---|---|---|---|
| Froxel 160×90×64 | 922k | ~14M | 15 MB | visibly soft |
| Froxel 240×135×96 | 3.1M | ~47M | 50 MB | better, still soft |
| Froxel 320×180×128 | 7.4M | ~110M | 118 MB | approaching Half |
| Half raymarch | — | ~96M | none | crisp |

The conclusion the table forces: **at the resolution needed to match Half's quality, the
froxel path costs about the same and carries a nine-figure byte count of volume with it.**

`Half` holds its edges at a quarter of the pixel count because its upsample is *bilateral* —
the `1 / (eps + |fullEye - halfEye|)` term explicitly rejects taps across depth
discontinuities, which is exactly where a cone boundary lives. Trilinear filtering of a grid
has no equivalent and blurs straight through them.

**Use Froxel when** fixture counts are high enough that the raymarch's per-pixel light loop
dominates, or when the look wanted is soft haze rather than defined beams. Its cost is flat
in screen resolution, which is the one axis where it genuinely wins.

**Use Half otherwise**, and `Full` for capture where upsample artefacts matter more than cost.

Tuning froxel if you do use it: raise `froxelResolution` until edges are acceptable and
measure the result, and raise `froxelNearDistance` to roughly where your rig starts —
slices are spaced exponentially, so metres spent in front of the stage are resolution
thrown away.
