...
- **Surfaces are lit properly.** Each fixture lights the floor, the walls and the avatars with the surface's own colour and gloss, and screen-space contact shadows are available if you want them. Many fixtures cost about what a few would in Unity's own light system, because there is no shadow map per light.
- **Beams in the air.** The cones are raymarched, with gobos, haze and strobe, and they sit correctly against the geometry in front of them and behind them.
- **Your renderer is left alone.** The package injects its passes at runtime and never edits your URP asset or renderer, which lets it work in projects where the renderer is not yours to change.
- **DMX or audio.** DMX from a desk can arrive as a video grid or as bytes inside a live video stream, and AudioLink can drive a rig from the music with no desk at all.
...
- A desktop-class GPU. The package uses compute shaders and a full-screen raymarch, so it is built for desktop VR and flatscreen rather than for Quest.
...
There is no renderer feature to add and no URP setting to find. After you install, it is worth running **VRSL → URP → Validate Renderer Setup** once. It reads your pipeline asset and the open scene and says in plain terms whether anything needs to change, without changing anything itself.
...
1. **Open an example scene.** See a working rig before you build your own.
   ...
   The AudioLink scene lights up as soon as audio plays. The DMX scenes need a desk or a DMX sender to feed them, and until one does they stay dark, which is the expected state rather than a fault.
...
4. **Patch them.** Give each DMX fixture the universe and start channel that match the desk. The prefabs use VRSL's 13-channel layout, and the five-channel statics have a layout of their own. An AudioLink fixture takes a frequency band and a colour source instead.

5. **Press play.** Surfaces should light and beams should appear. If they do not, select the manager, right-click its header and run **VRSL Diagnostics**, which says which stage went quiet.
...
The controls that set what the package costs sit on the manager, and there are few of them on purpose. The artistic controls, such as density, tint, anisotropy, intensity and fog coupling, change the look at a fixed price, so you can set those however the scene wants them.
...
| **Secondary cameras** | `Match`, `Reduced`, `SurfaceOnly`, `Skip` | What mirrors, portals and camera props get. Each one runs the whole light path again, so this is where a world with mirrors spends or saves. `Reduced`, the default, lights them one level below the scene, so the beams stay at a lower price. `SurfaceOnly` keeps the surface lighting in mirrors and drops the beams, and `Skip` leaves mirrors unlit by VRSL altogether. |
...
|---|---|
| **VRSL → URP → Validate Renderer Setup** | Which renderer each camera uses, its depth-priming and MSAA modes, and whether the prepass covers the layers your fixtures sit on. Also what each camera renders at under the secondary-camera policy, and any opaque shader that lacks a depth pass. It only reads. |
...
Start with **VRSL Diagnostics** on the manager. Most faults in a lighting pipeline look the same from outside, in that nothing is lit, and the report is what tells them apart.

| Symptom | Likely cause |
|---|---|
| Nothing lit at all | Most often a full-screen shader that did not compile; **Validate Shaders** will say. |
...
| DMX values plausible but wrong | Usually addressing. A fixture patched at the wrong start channel reads its neighbour's channels and still looks deliberate. Open the DMX Monitor: a patch off by one shears diagonally across the grid. Check that `use5ChannelMode` matches the patch. |
| A DMX fixture is half as bright as in an older version | This is deliberate: `maxIntensity` is now on the same scale as a URP spot light's Intensity. Double it, or set `curveMod` to 1. |
...
- **Bytes.** Where the values already arrive as data, a channel source hands them straight to the manager, with no encoding or decoding in between. [Truss](https://github.com/towneh/Truss) carries a desk's Art-Net inside a live H.264 stream, and a Basis media player showing that stream can feed the rig through **VRSL → URP → DMX Config → Add Basis DMX Record Source (SEI)**. The fixtures then read the exact bytes the desk sent, each record carries a checksum, and none of the picture is given up to a grid.

[DMX Channel Sources](https://github.com/towneh/vrsl-urp/wiki/DMX-Channel-Sources) covers both routes, along with how to frame a grid that arrives inside a larger picture and how to write a source of your own.
