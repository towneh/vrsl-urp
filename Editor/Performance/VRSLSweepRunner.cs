...
namespace VRSL.URP.EditorScripts
{
    ...
    static class VRSLSweepRunner
    {
        ...
        /// MonoBehaviour, which is fine because a sweep never runs in a player.</summary>
        class Host : MonoBehaviour
        {
            ...
            IEnumerator RunSweep()
            {
                ...
                var run = NewRun("standard-sweep");
                var root = GameObject.Find(VRSLBenchmarkScene.RootName);
                var camera = VRSLBenchmarkScene.FindCamera(root);

                if (root == null || camera == null)
                {
                    Finish(null, "The sweep scene is not in the loaded scene. Build it and try again.");
                    yield break;
                }
                ...
                if (manager == null)
                {
                    Finish(null, "No VRSL DMX light manager in the sweep scene.");
                    yield break;
                }
                ...
                if (source == null)
                {
                    Finish(null, "The sweep scene has no synthetic DMX source, so the fixtures "
                               + "would have nothing to light from.");
                    yield break;
                }
                manager.ChannelSource = source;

                // Everything that can refuse the run happens above this line. A fixed
                // target rather than the Game view, so the measurement does not depend on
                ...
                // exception anywhere inside propagates out of the coroutine, and without
                // this Finish never runs: play mode never ends, nothing reaches
                // SessionState, and the window waits for a result that is not coming.
                //
                ...
                // The size actually rendered, which is not the Game view's.
                ...
                foreach (int wanted in VRSLBenchmarkScene.FixtureCounts)
                {
                    ...
                    foreach (var variant in VRSLBenchmarkScene.CameraVariants)
                    {
                        ...
                        foreach (var level in VRSLQualityPreset.All)
                        {
                            ...
                            // Deliberately a log rather than a progress bar. A
                            // progress bar repaints the editor while it is up, and
                            // the thing being measured here is how long a frame
                            // takes.
                ...
                finally
                {
                    ...
                    // lights it switched off may not have been its own.
                    VRSLBenchmarkScene.RestoreScene();
                    if (!completed)
                        Finish(null, "The sweep stopped before it finished. The Console holds "
            ...
            static void StampEnvironment(VRSLBenchmarkRun run)
            {
                ...
                VRSLPerfReport.StampPackage(run.environment);
            ...
            void Finish(VRSLBenchmarkRun run, string error)
            {
                ...
                if (run != null)
                {
                    ...
                    try { SessionState.SetString(ResultKey, VRSLPerfReport.Write(run)); }
    ...
    /// <summary>Where reports go, and what goes at the top of them.</summary>
    static class VRSLPerfReport
    {
        ...
        public static string Folder(string label)
        {
            ...
            string folder  = Path.Combine(project, "VRSL-Benchmarks", $"{stamp}_{label}");
