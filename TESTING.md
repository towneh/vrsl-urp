...
| D2 | Both | Priming `Disabled`, same scene | Identical result. **Passed 2026-08-31 at MSAA 2x**, and this is the first run in which it actually varied anything: 1761 px of 262144 differing (0.672%) against a 1.0% budget, lit fraction 62.89% against 62.99% against a 0.5 tolerance. Expect a wider scatter here than the ~0.23% recorded at MSAA 1 — an `Equal` test against a multisampled prepass disagrees at more boundaries, and `p99 0.0039` says 99% of those pixels are off by one value of 255 with only a thin tail reaching the 0.6275 max. **The automated half reports `Inconclusive` rather than passing when the two captures come back bit-identical**, because that means priming never varied and the row compared one configuration with itself. It deliberately does not predict from MSAA or rendering mode whether priming will run: those conditions live in the URP a project ships, and a row that predicts wrong goes quiet instead of red — which is how this row spent its first weeks skipping on the one project that matters |
