...
| H14 | — | **A-M0-7, the stopwatch.** A full before-and-after: capture, change the package, capture, compare | Under fifteen minutes of wall clock and under two minutes of attention. Measured 2026-08-27 through `bench.sh`, with the lever a doubling of the Standard preset's volumetric step count: see the figures in the programme's `results.md`. **Time the steady state, not the first run** — the first build of a player on a machine is a cold IL2CPP build of the whole host project, and the first launch pays for shader and driver caches that every later one reuses. Neither recurs within a milestone |
