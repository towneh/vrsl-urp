...
| C8 | — | `VRSLManagerLifecycleTests` in the suite: a second manager left switched on beside the one that owns the singleton | The owner's channel-count global survives. Per-frame work is the owner's alone: a non-owner reaching `UploadChannels` with no source of its own rebinds the channel buffer and zeroes the channel-count global, so it blanks the owner's channels every frame with nothing in the Console. The row pins the global rather than the picture — the rig's channel readback binds the owner's own buffer explicitly and cannot see a global being taken over — so **the visible consequence in a real scene is still worth an eye** |
