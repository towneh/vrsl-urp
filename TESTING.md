...
| N11 | D | Profiling scene with movers, channel source on Ramp, `universes = 4`. Log immediately after entering Play, then again after several seconds | The `dir` field must **move between the two** and then settle. Movement buffers start zeroed, so every fixture damps from a standing start toward the value its pan and tilt channels hold, and Ramp holds them still — so the steady state is reached and stays. A `dir` identical in both samples means the damping never ran; a `dir` still drifting minutes later means it is not converging |
| N12 | D | Same, but compare fixtures whose smoothness channel differs. Channel 13 of a sector is flat `((ch - 1) / 13 + 1) * 13` | Fixtures whose smoothness channel reads high converge **sooner**, because smoothing lerps from `movementSmoothingMax` at 0 to `movementSmoothingMin` at full, and a smaller smoothing constant reaches the target faster. Sampling during the first second or two separates them; once everything has settled they are indistinguishable, so this row has to be run promptly after Play |
