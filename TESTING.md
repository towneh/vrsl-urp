...
| N2 | D | Same source in Fixtures mode, no video and no capture camera running | Fixtures **light** from the buffer alone: `Log Decoded Fixture Light Data` shows non-zero intensity and the buffer's colours. They do **not** move from it. Colour, intensity, cone and gobo selection read the buffer; pan, tilt and strobe still read their CRTs, because those carry state the chain accumulates rather than DMX values. Movement arrives with the state work on `feat/dmx-state-in-compute`, and this row wants rewriting to "light and move" when it does |
...
| N11 | D | Profiling scene with movers, channel source on Ramp, `universes = 4`. Log about a second after entering Play, then again after ten | The `dir` field must differ between the two samples for at least some fixtures, and the second sample is the settled state. Movement buffers start zeroed, so every fixture damps from a standing start toward what its pan and tilt channels hold; Ramp holds those still, so the target never moves and the value arrives and stays. Identical `dir` in both samples means the damping never ran. `dir` still changing between two late samples means it is not converging |
| N12 | D | The same pair of logs, compared per fixture | How far a fixture has travelled by the first sample must follow its own smoothness channel, which is channel 13 of its sector — flat `((ch - 1) / 13 + 1) * 13`, or simply `absChannel + 12`. Under Ramp at 50 fixtures the time constants run from about 0.14 s to 0.82 s, a spread of six. At one second the fast fixtures — (016), (017), (018), (037) among them, whose smoothness channels read near full — are within 0.1% of their final `dir` and so identical in both samples. The slow ones — (000), (019), (038), (039) — still have a fifth to a third of the journey left and must visibly differ. No sub-second timing is needed: it is the *difference between the two samples*, per fixture, that carries the result |
