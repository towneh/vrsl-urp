...
| D4 | Both | Priming `Forced`, MSAA 4x | Identical result — and **check which of two rows this is before reading it.** Stock URP refuses to prime on a multisampled target, so `Forced` plus MSAA renders as though priming were `Disabled`, and the row only shows that raising MSAA breaks nothing. A project may ship a URP without that guard, and then priming really is running at MSAA and the row is a genuine priming test. **Basis is the second case**: it vendors URP as an embedded package with the MSAA condition patched out (`UniversalRendererRenderGraph.IsDepthPrimingEnabledRenderGraph`, marked `BASISVR MODIFIED`), so priming runs there at MSAA 2x. Do not assume the stock behaviour — read the URP the project actually resolves, which is `Packages/` before the registry |
